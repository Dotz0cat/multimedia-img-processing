...
static void noise_reduction_lchuv(struct jpeg_img_data *restrict img) {
    ...
    #pragma omp parallel for shared(image, new_image) firstprivate(neighbors, mask, average) collapse(2)
    for (size_t i = 0UL; i < img->height - 1; i++) {
        for (size_t j = 0UL; j < img->width - 1; j++) {
            //#pragma omp task
            {
                mask |= (i > 0 && j > 0) ? 1u << 0u : 0u;
                neighbors[0]  = (mask & (1u << 0u)) ? image[ ( ( i - 1 ) * img->width + ( j - 1 ) ) * 3 + 0] : -1;
                neighbors[1]  = (mask & (1u << 0u)) ? image[ ( ( i - 1 ) * img->width + ( j - 1 ) ) * 3 + 1] : -1;
                neighbors[2]  = (mask & (1u << 0u)) ? image[ ( ( i - 1 ) * img->width + ( j - 1 ) ) * 3 + 2] : -1;

                mask |= (i > 0) ? 1u << 1u : 0u;
                neighbors[3]  = (mask & (1u << 1u)) ? image[ ( ( i - 1 ) * img->width + ( j + 0 ) ) * 3 + 0] : -1;
                neighbors[4]  = (mask & (1u << 1u)) ? image[ ( ( i - 1 ) * img->width + ( j + 0 ) ) * 3 + 1] : -1;
                neighbors[5]  = (mask & (1u << 1u)) ? image[ ( ( i - 1 ) * img->width + ( j + 0 ) ) * 3 + 2] : -1;

                mask |= (i > 0 && j < img->width - 1) ? 1u << 2u : 0u;
                neighbors[6]  = (mask & (1u << 2u)) ? image[ ( ( i - 1 ) * img->width + ( j + 1 ) ) * 3 + 0] : -1;
                neighbors[7]  = (mask & (1u << 2u)) ? image[ ( ( i - 1 ) * img->width + ( j + 1 ) ) * 3 + 1] : -1;
                neighbors[8]  = (mask & (1u << 2u)) ? image[ ( ( i - 1 ) * img->width + ( j + 1 ) ) * 3 + 2] : -1;

                mask |= (j > 0) ? 1u << 3u : 0u;
                neighbors[9]  = (mask & (1u << 3u)) ? image[ ( ( i + 0 ) * img->width + ( j - 1 ) ) * 3 + 0] : -1;
                neighbors[10] = (mask & (1u << 3u)) ? image[ ( ( i + 0 ) * img->width + ( j - 1 ) ) * 3 + 1] : -1;
                neighbors[11] = (mask & (1u << 3u)) ? image[ ( ( i + 0 ) * img->width + ( j - 1 ) ) * 3 + 2] : -1;

                mask |= (j < img->width -1) ? 1u << 4u : 0u;
                neighbors[12] = (mask & (1u << 4u)) ? image[ ( ( i + 0 ) * img->width + ( j + 1 ) ) * 3 + 0] : -1;
                neighbors[13] = (mask & (1u << 4u)) ? image[ ( ( i + 0 ) * img->width + ( j + 1 ) ) * 3 + 1] : -1;
                neighbors[14] = (mask & (1u << 4u)) ? image[ ( ( i + 0 ) * img->width + ( j + 1 ) ) * 3 + 2] : -1;

                mask |= (i < img->height - 1 && j > 0) ? 1u << 5u : 0u;
                neighbors[15] = (mask & (1u << 5u)) ? image[ ( ( i + 1 ) * img->width + ( j - 1 ) ) * 3 + 0] : -1;
                neighbors[16] = (mask & (1u << 5u)) ? image[ ( ( i + 1 ) * img->width + ( j - 1 ) ) * 3 + 1] : -1;
                neighbors[17] = (mask & (1u << 5u)) ? image[ ( ( i + 1 ) * img->width + ( j - 1 ) ) * 3 + 2] : -1;

                mask |= (i < img->height - 1) ? 1u << 6u : 0u;
                neighbors[18] = (mask & (1u << 6u)) ? image[ ( ( i + 1 ) * img->width + ( j + 0 ) ) * 3 + 0] : -1;
                neighbors[19] = (mask & (1u << 6u)) ? image[ ( ( i + 1 ) * img->width + ( j + 0 ) ) * 3 + 1] : -1;
                neighbors[20] = (mask & (1u << 6u)) ? image[ ( ( i + 1 ) * img->width + ( j + 0 ) ) * 3 + 2] : -1;

                mask |= (i < img->height - 1 && j < img->width - 1) ? 1u << 7u : 0u;
                neighbors[21] = (mask & (1u << 7u)) ? image[ ( ( i + 1 ) * img->width + ( j + 1 ) ) * 3 + 0] : -1;
                neighbors[22] = (mask & (1u << 7u)) ? image[ ( ( i + 1 ) * img->width + ( j + 1 ) ) * 3 + 1] : -1;
                neighbors[23] = (mask & (1u << 7u)) ? image[ ( ( i + 1 ) * img->width + ( j + 1 ) ) * 3 + 2] : -1;
            }
            ...
            //#pragma omp task
            {
                ...
                */
                ...
                average[0] += image[ ( i * img->width + j ) * 3 + 0 ];
                average[1] += image[ ( i * img->width + j ) * 3 + 1 ];
                average[2] += sin(image[ ( i * img->width + j ) * 3 + 2 ]);
                average[3] += cos(image[ ( i * img->width + j ) * 3 + 2 ]);

                average[0] /= stdc_count_ones_uc(mask) + 1;
                average[1] /= stdc_count_ones_uc(mask) + 1;
                average[2] = atan2(average[2], average[3]);
            }

            //#pragma omp task
            {
                new_image[ ( i * img->width + j ) * 3 + 0] = (fabs(image[(i * img->width + j) * 3 + 0] - average[0]) / sqrt(img->mse[0]/(img->height * img->width)) > 1.0) ? average[0] : image[(i * img->width + j) * 3 + 0];
                new_image[ ( i * img->width + j ) * 3 + 1] = (fabs(image[(i * img->width + j) * 3 + 1] - average[1]) / sqrt(img->mse[1]/(img->height * img->width)) > 1.0) ? average[1] : image[(i * img->width + j) * 3 + 1];
                new_image[ ( i * img->width + j ) * 3 + 2] = (fabs(image[(i * img->width + j) * 3 + 2] - average[2]) / sqrt(img->mse[2]/(img->height * img->width)) > 1.0) ? average[2] : image[(i * img->width + j) * 3 + 2];
            }
        }
    }
        ...
        for (size_t j = 0UL; j < img->width - 1; j++) {
            ...
            //#pragma omp task
            {
                ...
                */
                ...
                average[0] += image[ ( i * img->width + j ) * 3 + 0 ];
                average[1] += image[ ( i * img->width + j ) * 3 + 1 ];
                average[2] += image[ ( i * img->width + j ) * 3 + 2 ];

                average[0] /= stdc_count_ones_uc(mask) + 1;
                average[1] /= stdc_count_ones_uc(mask) + 1;
                average[2] /= stdc_count_ones_uc(mask) + 1;
            }

            //#pragma omp task
            {
                new_image[ ( i * img->width + j ) * 3 + 0] = ( fabs(image[ (i * img->width + j ) * 3 + 0] - average[0]) / sqrt(img->mse[0]/(img->height * img->width)) > 2.0) ? average[0] : image[(i * img->width + j) * 3 + 0];
                new_image[ ( i * img->width + j ) * 3 + 1] = ( fabs(image[ (i * img->width + j ) * 3 + 1] - average[1]) / sqrt(img->mse[1]/(img->height * img->width)) > 2.0) ? average[1] : image[(i * img->width + j) * 3 + 1];
                new_image[ ( i * img->width + j ) * 3 + 2] = ( fabs(image[ (i * img->width + j ) * 3 + 2] - average[2]) / sqrt(img->mse[2]/(img->height * img->width)) > 2.0) ? average[2] : image[(i * img->width + j) * 3 + 2];
            }
        }
    }
...
static void noise_detection_lchuv(struct jpeg_img_data *restrict img) {
    ...
    for (size_t i = 0UL; i < img->height - 1; i++) {
        for (size_t j = 0UL; j < img->width - 1; j++) {
            ...
            //#pragma omp task
            {
                new_image[ ( i * img->width + j ) * 3 + 0] = (fabs(image[(i * img->width + j) * 3 + 0] - average[0]) / sqrt(img->mse[0]) > 2.0) ? 0.0 : 100.0;
                new_image[ ( i * img->width + j ) * 3 + 1] = (fabs(image[(i * img->width + j) * 3 + 1] - average[1]) / sqrt(img->mse[1]) > 2.0) ? 0.0 : 100.0;
                new_image[ ( i * img->width + j ) * 3 + 2] = (fabs(image[(i * img->width + j) * 3 + 2] - average[2]) / sqrt(img->mse[2]) > 2.0) ? 0.0 : average[2];
            }
        }
    }
...
static void noise_detection_rgb(struct jpeg_img_data *restrict img) {
    ...
    for (size_t i = 0UL; i < img->height - 1; i++) {
        for (size_t j = 0UL; j < img->width - 1; j++) {
            ...
            //#pragma omp task
            {
                new_image[ ( i * img->width + j ) * 3 + 0] = ( fabs(image[ (i * img->width + j ) * 3 + 0] - average[0]) / sqrt(img->mse[0]) > 2.0) ? 0u : 255u;
                new_image[ ( i * img->width + j ) * 3 + 1] = ( fabs(image[ (i * img->width + j ) * 3 + 1] - average[1]) / sqrt(img->mse[1]) > 2.0) ? 0u : 255u;
                new_image[ ( i * img->width + j ) * 3 + 2] = ( fabs(image[ (i * img->width + j ) * 3 + 2] - average[2]) / sqrt(img->mse[2]) > 2.0) ? 0u : 255u;
            }
        }
    }
